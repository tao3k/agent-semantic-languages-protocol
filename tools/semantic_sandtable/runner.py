...
RANK_PREFIXED_PATH_PATTERN = re.compile(
    ...
    validate_line_protocol_path_values(result, lines)


def validate_line_protocol_path_values(result: StepResult, lines: list[str]) -> None:
    for line in lines:
        ...
        if location_field is not None:
            value = location_field.group(1)
            validate_rank_prefixed_path_token(result, value, line)
            if result.errors:
                return
            validate_project_path_token(result, value, line)
            if result.errors:
                return
        owner_match = re.match(r"^\|owner\s+(\S+)", line)
        if owner_match is not None:
            validate_project_path_token(result, owner_match.group(1), line)
            if result.errors:
                return
        edge_match = re.match(r"^\|edge\s+(\S+)\s+-[^ ]+->\s+(\S+)", line)
        if edge_match is not None:
            for token in edge_match.groups():
                validate_rank_prefixed_path_token(result, token, line)
                if result.errors:
                    return
        for match in re.finditer(r"\bat=([^,\s)]+)", line):
            validate_rank_prefixed_path_token(result, match.group(1), line)
            if result.errors:
                return
...
def validate_rank_prefixed_path_token(result: StepResult, value: str, line: str) -> None:
...
def validate_project_path_token(result: StepResult, value: str, line: str) -> None:
    if PROJECT_PATH_PATTERN.fullmatch(value) is not None:
        return
    result.errors.append(f"line protocol invalid project path {value!r} in {line[:80]!r}")
